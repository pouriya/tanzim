...
- `DeepMerge` — built-in: maps with the same name are merged recursively. Same-name lists follow a configurable `ArrayStrategy` (default `Replace`; also `Concat`/`Prepend`/`Union`/`Index`/`Keyed`), set via `DeepMerge::new().with_array_strategy(..)`. Overlay `Value::Null` at a map key deletes that key; otherwise the overlay value/location wins at every other leaf.
